...
I have conducted an extensive analysis of trade data from 2015 to 2022, focusing on countries involved in trade with Ukraine. The main objective was to examine the trade relationships and dynamics between Ukraine and its trading partners during this specific time period. To achieve this, I created a trade network matrix consisting of 209 countries that have trade relations with countries trading with Ukraine, allowing us to observe changes in international trade.

The trade data includes all the countries that engaged in import and export activities with the countries trading with Ukraine between 2015 and 2022. This comprehensive dataset enables an in-depth analysis of the trade changes resulting from the ongoing conflict. By examining the trade relationships within this context, we can gain valuable insights into the impact of the war on international trade dynamics.
...
# Load Data, and Identify Trading Countries with Ukraine: 
...
Group countries based on trade intensity: Group countries that are importing wheat from ukraine based on their trade intensity with Ukraine. Categorize countries into three groups: high trade intensity, moderate trade intensity, and low trade intensity.
...
consolidated_export <- data.frame(country = character(), export = numeric(), stringsAsFactors = FALSE)
...
cat("-------------------------------------------------------------------------\n")
cat("               Ukraine Trade Data Cluster Analysis                     \n")
cat("-------------------------------------------------------------------------\n")
cat("\n")
cat("-------------------------------------------------------------------------\n")
...
cat("  Country          Consolidated Export   Intensity  \n")
cat("-------------------------------------------------------------------------\n")
...
for (i in 1:length(years)) {
  ...
  # Print the sorted and clustered trade data for the current year
  cat("----------------------------------\n")
  cat("     Sorted and Clustered Trade Data - Year", year, "     \n")
  cat("----------------------------------\n")
  cat("\n")
  cat("  Country          Export    Intensity  \n")
  cat("----------------------------------\n")
  
  # Get the sorted and clustered trade data for the current year
  sorted_trade_year <- sorted_trade_list[[as.character(year)]]
  
  # Iterate over the rows in the sorted_trade_year data frame
  for (k in 1:nrow(sorted_trade_year)) {
    # Format the country, export, and trade_intensity values
    country <- sprintf("%-20s", sorted_trade_year$country[k])
    export <- sprintf("%-10s", sorted_trade_year$export[k])
    intensity <- sprintf("%-15s", sorted_trade_year$trade_intensity[k])
...
Prepare trade network for the countries importing wheat from Ukraine, to analyze trade disruption. Focus will be on the countries in the high trade intensity group and assess the impact of the war on their wheat trade with Ukraine and Other Countries.
...
Prepare Network Data for the Countries, who imported wheat from Ukraine between year 2015-202 and save in the directory

```{r}
# Create an empty list to store trade data matrices for each year
trade_data_matrices <- list()
...
for (year in years) {
  ...
  trade_data <- data %>% 
    dplyr::select(all_countries_alys)
  trade_data <- trade_data[c(all_countries_alys),]
  
  # Add the trade data matrix for the current year to the list
  trade_data_matrices[[as.character(year)]] <- trade_data
}

# Specify the destination folder to save the trade data files
destination_folder <- "C:/social network project/project data/1001/Unform Data/Merged/TradeData/"

# Create the destination folder if it doesn't exist
dir.create(destination_folder, showWarnings = FALSE)

# Loop through each year
for (i in 1:length(trade_data_matrices)) {
  year <- years[i]
  trade_data <- trade_data_matrices[[i]]
  
  # Save the trade data matrix for the current year
  file_name <- paste0(destination_folder, "TradeData_", year, ".csv")
  write.csv(trade_data, file = file_name, row.names = TRUE)
}

```

Sociogram of the trade network of the countries, who imported wheat from Ukraine between year 2015-2022

```{r}
# Define the file paths and the years
file_paths <- paste0("C:/social network project/project data/1001/Unform Data/Merged/TradeData/TradeData_", 2015:2022, ".csv")
years <- 2015:2022
...
for (i in seq_along(file_paths)) {
  ...
  adjacency_matrix <- as.matrix(read.csv(file_paths[i], row.names = 1))
...
Year wise High Intensity, Normal Intensity, Low Intensity Trade Countries
...
print(high_intensity_countries_year["2015"])
```
...
Network Graph on consolidated trade from year 2015 to 2022, for the trade network created for the countries importing wheat from ukraine, also plotting High Intensity Trade, Normal Intensity Trade and Low Intensity Trade
...
Network Graph for top 100 countries by consolidated trade from year 2015 to 2022, for the trade network created for the countries importing wheat from ukraine, also plotting High Intensity Trade, Normal Intensity Trade and Low Intensity Trade
...
Years wise, Calculation of Network Attributes including Nodes, Edges, Bipartite, Directed, Weighted, Connected, Components, Components Size, Diameter, Average Path Length, Giant Component Proportion, Singleton Proportion, Vertex Attribute Names, Edge Attribute Names, Max Edge Weights, Min Edge Weights, Median Edge Weight, Edge Density.
...
## Diad and Triad Analysis for High Trade Countries:
...
## Year wise, Country wise Network Attributes: 

Degree, In_Degree, Out_Degree, Betweenness Centrality, Eigenvector Centrality, Closeness Centrality, Constraint Centrality, In_Weights, Out_Weights etc.
...
filtered_df_2015 <- degree_df %>% filter(year == 2015, consol_intensity == "high")
filtered_df_2016 <- degree_df %>% filter(year == 2016, consol_intensity == "high")
filtered_df_2017 <- degree_df %>% filter(year == 2017, consol_intensity == "high")
filtered_df_2018 <- degree_df %>% filter(year == 2018, consol_intensity == "high")
filtered_df_2019 <- degree_df %>% filter(year == 2019, consol_intensity == "high")
filtered_df_2020 <- degree_df %>% filter(year == 2020, consol_intensity == "high")
filtered_df_2021 <- degree_df %>% filter(year == 2021, consol_intensity == "high")
filtered_df_2022 <- degree_df %>% filter(year == 2022, consol_intensity == "high")
...
for (i in seq_along(networks)) {
  ...
  ggsave(paste("individual_degree_distribution_plot_", year, ".png", sep = ""), individual_plot, width = 8, height = 6, dpi = 300)
...
'
Analyse Change in International Wheat Trade in 2022 through change in Degree, In_Degree, Out_Degree, Betweenness Centrality, In_Weight and Out_Weight; 
...
for (i in seq_along(networks)) {
  ...
  # Interpretation
  # Analyze the network visualization and identify the countries highlighted in red.
  # These are the prominent countries with high betweenness centrality, indicating their importance as intermediaries or brokers in the network.
  # They play a critical role in connecting different clusters or communities in the network.
  
  # Perform additional analysis or calculations as needed for your specific research question.
  
}
```

## Analysis Change in International Wheat Trade in 2022 

Analysis Change in International Wheat Trade in 2022 through change in Betweenness Centrality; Trade between Ukraine and new entrant in top 10 of betweeness in 2022:"Sweden", "Greece", "Israel", "Spain", "Ireland" and "Italy" are the new countries.
...
## Analysis though Eigenvector Centrality to Analyse Trade Disruption

Analysis though Eigenvector Centrality to identify if the prominent countries Australia, China, United States of America, Canada, Indonesia, Philippines, Russia, Egypt, Turkey, Japan, South_Korea, Mexico, France are importing from Ukraine?
```{r}
# List of target countries
...
# Analysis to find, which prominent countries as per Eigenvector Centralities are importing from "Spain", "Italy", "Turkey", "Poland", "Greece", "Romania", who are the new entrant in top 10, betweenness centrality measurement ranking

```{r}
# Read the data into a DataFrame
data_1 <- read_csv("C:/social network project/project data/1001/Unform Data/Merged/TradeData/TradeData_2022.csv")
...
for (i in seq_along(networks)) {
  ...
  community <- cluster_walktrap(network_undirected)
  ...
  g <- graph_from_adjacency_matrix(as_adjacency_matrix(network_undirected))
  
  # Set the vertex color based on community membership
  vertex_color <- rainbow(length(unique(community_membership)))[community_membership]
  
  # Plot the graph with node colors representing communities
  plot(g, vertex.color = vertex_color, vertex.label = NA, edge.arrow.size = 0.5, main = paste0("Year: ", year))
  
  # Filter the degree data frame for the current year's nodes
  temp <- degree_df[degree_df$year == year & degree_df$country %in% V(g)$name, ]
  
  # Match the community membership to the degree data frame
  temp$community <- community_membership[match(temp$country, V(g)$name)]
...
##Community Detection using walktrap community/cluster_walktrap/Fast Greedy Algorithm for the Countries Trading with Ukraine
...
```
## Community Plot of High, Normal, Low Intensity Trade Countries 
...
## CUG Test on Network from Year 2015 to 2022:
...
for (i in seq_along(networks)) {
  ...
  # Convert the network to the 'network' format
  # g <- asNetwork(network)

  # Fit the ERGM model
  
  trans.cug<-cug.test(network,FUN=gtrans,mode="digraph",cmode="size")
  

  # Summarize the ERGM model
  trans.cug

  # Plot the ERGM network
  plot(trans.cug, main = paste("Year:", year, "ERGM Network"))


}

```
## Hypothesis: The network correlation between two networks with the same nodes but at different points in time is significantly different from zero, indicating a change in the network structure over time.

```{r}
# Convert adjacency matrices to regular matrices
network1_matrix <- as.matrix(as_adjacency_matrix(network1))
network2_matrix <- as.matrix(as_adjacency_matrix(network2))
...
The Ukraine and Russia conflict has triggered significant disruptions in the global supply chain, with specific implications for wheat production, grain exports, and energy supply. The reliance on these countries as major suppliers of wheat and barley, as well as their involvement in other critical industries, has exposed vulnerabilities in various regions. As countries seek alternative sources, commodity prices rise, and inflation escalates, finding sustainable solutions and diversifying supply
...
Based on the 2022 data, it can be concluded that these prominent countries are involved in wheat imports from countries that have direct trade relationships with Ukraine. Furthermore, European countries with high betweenness centrality values serve as important intermediaries, importing wheat from Ukraine and exporting it to other prominent wheat-importing nations worldwide.
